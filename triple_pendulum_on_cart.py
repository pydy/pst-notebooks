# %%
r"""
Upright a Triple  Pendulum
==========================

Objectives
----------

- Show the use of opty's variable time interval feature to.
...
A triple pendulum is rotationally attached to a cart which can move along the
horizontal X axis. The goal is to get the triple pendulum to an upright
...
- The initial guess stored in the file named
  ``triple_pendulum_on_cart_no_h_i_301_nodes_solution.csv`` is the solution of
  the problem without constraints on the accelerations of the pendulum at the
  end of the motion. It takes over one hour on a regular PC to solve this
  problem.
- By declaring the :math:`h_i` as either state variables, or specifieds, one
  can see that declaring them as specifieds generally results in much faster
  convergence.
...
- :math:`m_4`: mass of the third pendulum [kg]
...
- :math:`i_{ZZ2}`: moment of inertia of the second pendulum about its end point
  [kg*m^2]
- :math:`i_{ZZ3}`: moment of inertia of the third pendulum about its end point
  [kg*m^2]
...
- :math:`q_4`: angle of the third pendulum with respect to the second [rad]
...
- :math:`u_4`: angular speed of the third pendulum [rad/s]


**Specifieds**

- :math:`k_f`: force applied to the cart [N]
...
O.set_vel(N, 0)
q1, q2, q3, q4, u1, u2, u3, u4, kf = me.dynamicsymbols((f'q1, q2, q3, q4, u1,'
                                                       f'u2, u3, u4, kf'))
h1, h2, h3, h4 = me.dynamicsymbols('h1 h2 h3 h4')
lx, m1, m2, m3, m4, g, iZZ1, iZZ2, iZZ3 = sm.symbols((f'lx, m1, m2, m3 m4 g,'
                                                      f'iZZ1, iZZ2 iZZ3'))
...
A3.orient_axis(N, q4, N.z)
A3.set_ang_vel(N, u4 * N.z)
...
P4.set_pos(P3, lx * A3.x)
P4.v2pt_theory(P3, N, A3)
...
I3 = me.inertia(A3, 0, 0, iZZ3)
P4a = me.RigidBody('P4a', P4, A3, m4, (I3, P4))

bodies = [P1a, P2a, P3a, P4a]

loads = [(P1, kf * N.x - m1*g*N.y), (P2, -m2*g*N.y), (P3, -m3*g*N.y),
         (P4, -m4*g*N.y)]
...
constant_symbols = (lx, m1, m2, m3, m4, g, iZZ1, iZZ2, iZZ3)
...
prob.add_option('max_iter', 95000)

# %%
# fname containse the solution to a simpler problem, see notes above.
fname = f'triple_pendulum_on_a_cart_no_h_i_{num_nodes}_nodes_solution.csv'
zeit = time.time()
initial_guess = np.zeros(prob.num_free)
initial_guess1 = np.loadtxt(fname)
initial_guess[0:8*num_nodes] = initial_guess1[0:8*num_nodes]
initial_guess[11*num_nodes:12*num_nodes] = initial_guess1[8*num_nodes:
                                                          9*num_nodes]
...
for _ in range(1):
    ...
    print(f"time taken {time.time() - zeit: .3f} sec")
    print('Message from optimizer:', info['status_msg'])
    print('Iterations needed', len(prob.obj_value))
    print(f"Objective value {solution[-1]: .3e}, \n")

if h1 in state_symbols:
    print('h_i are state variables')
else:
    print('h_i are unknown input trajectories')
# %%
...
P4_x = np.empty(num_nodes)
P4_y = np.empty(num_nodes)
...
P4_loc = [me.dot(P4.pos_from(O), uv) for uv in [N.x, N.y]]
...
P4_loc_lam = sm.lambdify(qL + pL_vals, P4_loc, cse=True)

for i in range(num_nodes):
    q_1 = solution[i]
    q_2 = solution[i + num_nodes]
    q_3 = solution[i + 2 * num_nodes]
    q_4 = solution[i + 3 * num_nodes]
    u_1 = solution[i + 4 * num_nodes]
    u_2 = solution[i + 5 * num_nodes]
    u_3 = solution[i + 6 * num_nodes]
    u_4 = solution[i + 7 * num_nodes]

    P1_x[i], P1_y[i] = P1_loc_lam(q_1, q_2, q_3, q_4, u_1, u_2, u_3, u_4,
                                  *list(par_map.values()))
    P2_x[i], P2_y[i] = P2_loc_lam(q_1, q_2, q_3, q_4, u_1, u_2, u_3, u_4,
                                  *list(par_map.values()))
    P3_x[i], P3_y[i] = P3_loc_lam(q_1, q_2, q_3, q_4, u_1, u_2, u_3, u_4,
                                  *list(par_map.values()))
    P4_x[i], P4_y[i] = P4_loc_lam(q_1, q_2, q_3, q_4, u_1, u_2, u_3, u_4,
                                  *list(par_map.values()))


# needed to give the picture the right size.
xmin = min(np.min(P1_x), np.min(P2_x), np.min(P3_x), np.min(P4_x))
xmax = max(np.max(P1_x), np.max(P2_x), np.max(P3_x), np.max(P4_x))
ymin = min(np.min(P1_y), np.min(P2_y), np.min(P3_y), np.min(P4_y))
ymax = max(np.max(P1_y), np.max(P2_y), np.max(P3_y), np.max(P4_y))
...
def animate(i):
    ...
    wert_x = [P3_x[i], P4_x[i]]
    wert_y = [P3_y[i], P4_y[i]]
    line3.set_data(wert_x, wert_y)
...
fig, ax, line1, line2, line3, recht = animate_pendulum(times, P1_x, P1_y,
                                                       P2_x, P2_y)
