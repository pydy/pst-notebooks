...
- Show how to use the event feature of solve_ivp, which allows to stop the
  integration wen an event occurs, here a second spoke hits the road.
...
- Not all ll eventualities are considered. If, for example, the street is
...
- It may be noteworth, that the rotational speed of the wheel may change
...
# %%
# Set the geometry, define point, etc.
...
#=============================
if isinstance(n, int) != True or n < 3:
...
# ----------------------
# *value* is the number of the spoke presentlytouching the street
...
#
# If (as I do it her, of course), *event.terminal = True*,  solve_ivp stops
# when the event has happened and returns the results obtained so far.
# They are in *ergebnis*.
# THen the new :math:`frac{d}{dt}q(t)` for the new inital values, as in general
